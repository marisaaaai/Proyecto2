...
graph = Graph("http://neo4j:hola@127.0.0.1:7474/db/data")
...
if (op=="1"):
    ...
    while p<1:
        nombre=nombres.pop()
        listaDuracion=graph.run("MATCH (d:canal { name: '"+nombre+"' }) RETURN d.duracion").data()
        dicDuracion=listaDuracion.pop()
        duracion=dicDuracion.get('d.duracion')
        
        listaDinamicidad=graph.run("MATCH (di:canal { name: '"+nombre+"' }) RETURN di.esDinamico").data()
        dicDinamicidad=listaDinamicidad.pop()
        dinamicidad=dicDinamicidad.get('di.esDinamico')
        
        listaEjercicios=graph.run("MATCH (e:canal { name: '"+nombre+"' }) RETURN e.es_ejercicio").data()
        dicEjercicios=listaEjercicios.pop()
        ejercicio=dicEjercicios.get('e.es_ejercicio')
        
        listaTipo=graph.run("MATCH (t:canal { name: '"+nombre+"' }) RETURN t.tipoVideo").data()
        dicTipo=listaTipo.pop()
        tipo=dicTipo.get('t.tipoVideo')
    ...
    if(op=="2"):
        ...
        if(op2=="1"):
            ...
            graph.run("CREATE(n1:canal{name:'"+nombreCanal+"',link:'"+linkCanal+"',duracion:'"+duracionCanal+"',esDinamico:'"+dinamismoCanal+"',es_ejercicio:'"+ejercicioCanal+"',tipoVideo:'"+tipoCanal+"'})").data()
